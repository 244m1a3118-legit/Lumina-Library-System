...
export const setDoc = async (ref: any, data: any) => {
  ...
  await fetch(`${API_BASE}/db/${ref.path}/${ref.id}`, {
    method: 'PUT',
...
export const getDoc = async (ref: any) => {
  const res = await fetch(`${API_BASE}/db/${ref.path}/${ref.id}`);
...
export const deleteDoc = async (ref: any) => {
  await fetch(`${API_BASE}/db/${ref.path}/${ref.id}`, { method: 'DELETE' });
};
...
export const getDocs = async (ref: any) => {
  const res = await fetch(`${API_BASE}/db/${ref.path}`);
...
export const onSnapshot = (ref: any, callback: (snap: any) => void, errorCallback?: (err: any) => void) => {
  ...
  const fetchAndCallback = async () => {
    ...
    try {
      ...
      const url = isDoc ? `${API_BASE}/db/${ref.path}/${ref.id}` : `${API_BASE}/db/${ref.path}`;
